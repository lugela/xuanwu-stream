...
public final class JdbcDialectLoader {
     ...
     */
    public static JdbcSqlTemplate load(String url) {
        ClassLoader cl = Thread.currentThread().getContextClassLoader();
        List<JdbcSqlTemplate> foundFactories = discoverFactories(cl);

        if (foundFactories.isEmpty()) {
            throw new IllegalStateException(
                    String.format(
                            "Could not find any jdbc dialect factories that implement '%s' in the classpath.",
                            JdbcSqlTemplate.class.getName()));
        }

        final List<JdbcSqlTemplate> matchingFactories =
    ...
    private static List<JdbcSqlTemplate> discoverFactories(ClassLoader classLoader) {
        try {
            final List<JdbcSqlTemplate> result = new LinkedList<>();
            ServiceLoader.load(JdbcSqlTemplate.class, classLoader)
                    .iterator()
                    .forEachRemaining(result::add);
            return result;
        } catch (ServiceConfigurationError e) {
            LOG.error("Could not load service provider for jdbc dialects Template.", e);
            throw new RuntimeException(
                    "Could not load service provider for jdbc dialects Template.", e);
        }
    }
}
